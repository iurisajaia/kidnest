<div id="update-kid-{{$kid->id}}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full shadow bg-[#fff] rounded-lg">
        <!-- Modal content -->
        <div class="relative rounded-lg">
            <div class="px-6 py-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium">{{__('edit')}} - {{$kid->first_name}}</h3>
                <form class="space-y-6" action="{{route('kids.update', $kid->id)}}" method="POST">
                    @csrf
                    @method('PUT')
...
{{--                            </svg>--}}
{{--                        </div>--}}
{{--                        <input datepicker value="{{$kid->registration_date}}" type="text" placeholder="{{__('registration.date')}}" name="registration_date"--}}
...
{{--                    </div>--}}
                    ...
                    <div class="flex justify-end">
                        <div data-modal-hide="update-kid-{{$kid->id}}">
